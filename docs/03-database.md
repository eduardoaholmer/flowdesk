...
### 6.8 Adições da Sprint 9.4 (Sub-issues, ADR-059)

- **`issues.parent_id`** (`uuid`, nullable, migration `e2c2c5c8594a`) — auto-referência a `issues.id`, `ON DELETE RESTRICT` (mesmo padrão de `assignee_id`/`creator_id`, nunca aciona na prática porque a aplicação só soft-deleta). Sem `relationship()` ORM — nenhum consumidor precisa de eager-load de pai/filhos, o acesso é sempre via `IssueRepository` filtrando por `parent_id` (mesmo padrão de `project_id`). Limitado a **um único nível** de hierarquia (uma sub-issue não pode ter sub-issues): `IssueService` recusa vincular uma issue que já é pai (tem filhos) ou que já é filha (`parent_id` preenchido) — ver ADR-059. Índice `ix_issues_parent_id_deleted_at` (mesmo padrão de `project_id`/`assignee_id`/`creator_id`) para listar filhos de uma issue sem table scan.
...
| Issue → Issue (sub-issue) | 1:N, opcional, um único nível | `issue.parent_id`, `ON DELETE RESTRICT` (Sprint 9.4/ADR-059) — uma sub-issue não pode ter sub-issues, aplicado no service |
...
27. `e2c2c5c8594a` — `add_parent_id_to_issues` (Sprint 9.4/ADR-059): puramente aditiva — coluna `parent_id` (`nullable`) + FK `issues_parent_id_fkey` (`RESTRICT`, auto-referência) + índice `ix_issues_parent_id_deleted_at`. Sem backfill (issues existentes nascem com `parent_id NULL`, comportamento correto — nenhuma tinha sub-issue antes desta sprint).
