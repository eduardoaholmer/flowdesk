...
> **Nota (pós-implementação, Sprint 16.1/M5)**: `/w/:workspaceSlug/board` (`BoardPage`, `features/issues/components/IssuesBoardView.tsx`) — board somente-leitura por status, escopo de workspace (não de `Team`, ainda dormente — ver `docs/09-decision-log.md` ADR-033). Reaproveita `useIssues`/`GET /issues` já existente, sem endpoint novo; agrupa client-side pelos seis valores fixos de `IssueStatus`. Vive em `features/issues/` (não uma feature `board/` própria) por só orquestrar dado que `issues` já expõe via `hooks.ts`/`api.ts` — mesmo critério de fronteira de feature do §2.
