...
- ➡️ **M4 — Testing & Documentation debt** (atual): fechar a lacuna real de cobertura de teste do frontend (hoje mínima — só smoke tests, sem MSW) e a fase 2 da Sprint 9 (métricas básicas, revisão de segurança completa do checklist de `docs/07-security.md`). Candidato natural para também configurar Playwright/regressão visual (ver ressalva da Sprint 11).
- **M5 — Kanban Board**: `Team`/`WorkflowState` (schema dormente desde a Sprint 2/ADR-007, nunca removido) mais board com drag-and-drop — o antigo "Núcleo de Issues" original da Sprint 0/4/6 nunca executado nessa forma.
