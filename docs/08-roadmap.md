...
- **Dependências**: nenhuma.
- **DoD**: DoD-base satisfeita — `tsc`/`eslint`/`vitest` do frontend limpos; `ruff check`/`ruff format`/`mypy`/`pytest tests/unit` do backend rodados numa validação retroativa (ADR-042), 169/169 unitários passando (achou e corrigiu uma regressão real: `test_authorization.py` não previa a segunda exceção de ADMIN). Teste de contrato (`test_workspaces.py`) escrito seguindo o padrão existente, mas ainda não executado contra Postgres/Redis reais (indisponíveis no sandbox de validação).
...
- **DoD**: DoD-base satisfeita — `ruff check`/`ruff format`/`mypy`/`pytest tests/unit` rodados numa validação retroativa (ADR-042): achou e corrigiu um `ASYNC240` real em `S3StorageProvider.open()` (cleanup de arquivo temporário síncrono dentro de método async). `poetry.lock` regenerado com sucesso (resolve `boto3`/`boto3-stubs`). Teste de integração (`tests/integration/test_s3_storage_provider.py`) escrito, mas ainda não executado contra um MinIO real (sem Docker no sandbox de validação) — pendente validar o serviço `minio` novo em `docker-compose.yml`/CI num ambiente com Docker.
...
- **DoD**: DoD-base satisfeita — `ruff check`/`ruff format`/`mypy`/`pytest tests/unit` rodados numa validação retroativa (ADR-042), sem achado novo neste módulo especificamente. Verificação de que nenhum segredo de SMTP é logado feita por code review/docstring (`SMTPMailSender.send_password_reset` só loga `email`), não por teste automatizado (ADR-039, Desvantagens aceitas). Teste de integração (`tests/integration/test_smtp_mail_sender.py`) escrito, mas ainda não executado contra um MailHog real (sem Docker no sandbox de validação).
