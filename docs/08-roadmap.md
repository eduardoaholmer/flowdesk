...
- 🚧 **M5 — Kanban** (em andamento, aberto 2026-07-20, ADR-033): `Team`/`WorkflowState` (schema dormente desde a Sprint 2/ADR-007, nunca removido) mais board com drag-and-drop — o antigo "Núcleo de Issues" original da Sprint 0/4/6 nunca executado nessa forma. Deixou de ser considerado feature inicial de produto (`docs/00-product-vision.md` §5 original) — é agora uma feature premium construída só depois que o sistema inteiro estiver sólido (M2–M4 fechados). Quebrado em fases (Sprint 16.1–16.3): board somente-leitura em escopo de workspace com o status fixo atual (16.1, concluída — `Team` segue dormente), drag-and-drop (16.2), decisão explícita sobre reativar `Team`/workflow por time (16.3).
...
### Sprint 16.2 — M5: drag-and-drop de mudança de status
...
- **Dependências**: Sprint 16.1. Introduz uma biblioteca de drag-and-drop nova (`@dnd-kit/core`, candidata — `react-beautiful-dnd` está descontinuada) — dependência nova, mesma disciplina de check-in explícito de toda sprint que introduz dependência neste projeto.
- **DoD**: DoD-base.

### Sprint 16.3 — M5: decisão sobre `Team`/workflow configurável por time
