...
### Sprint 19.1 — M7: schema de Projetos/Labels (`Project.key`, `ProjectMember`, agregações de progresso/uso) (concluída)
...
- **Fechamento da verificação parcial (2026-07-21, ADR-050)**: validado desta vez contra um Postgres/Redis reais (não apenas `pytest tests/unit`). Achou e corrigiu um bug real: a migration `b3e7c1a9d240` adicionava `key` como `NOT NULL` direto, assumindo banco vazio (mesmo padrão da migration irmã `fc0a10c66145`, Sprint 6) — mas essa suposição não vale mais para `projects`, que já tinha linhas reais. Reescrita com o ciclo `nullable → backfill (derivação determinística do nome, mesmo espírito de `derive_project_key`) → NOT NULL`. Com a correção: `alembic upgrade head` → `downgrade base` → `upgrade head` limpo contra banco populado; suíte completa (`pytest`, unit+integration+contract) 367/367 verde — os 2 testes que falharam numa primeira rodada da suíte inteira (`test_admin_respects_limits`, `test_list_projects_paginates_and_returns_only_workspace_scoped`) passaram isoladamente e numa segunda rodada completa, confirmando flakiness de execução (não regressão do schema novo). `ruff`/`mypy` seguem limpos. Únicos erros remanescentes: os 3 testes de integração que dependem de MinIO/MailHog (Sprints 17.2/17.3) — serviços não disponíveis nesta sessão, limitação já documentada, sem relação com Projetos/Labels.
...
- **DoD**: DoD-base satisfeita — suíte completa (backend) verde contra banco/Redis reais; ciclo de migração reversível validado.

### Sprint 19.2 — M7: grid de cards de Projetos (concluída)

- **Objetivo**: substituir a listagem tabular de Projetos por um grid de cards consumindo `key`/`member_ids`/`issue_count`/`done_issue_count` (Sprint 19.1), fechando o gap visual de `Projetos e Labels.dc.html`.
- **Achado durante a sprint**: `ProjectFormFields` nunca teve campos para `key`/`lead_id`/`target_date` — settable via API desde a Sprint 2/6 (`lead_id`/`target_date`) ou desde a 19.1 (`key`), mas nenhuma tela os expunha. Sem isso, todo projeto novo nasceria sem líder/entrega/key custom e o grid ficaria com avatares/datas sempre vazios — corrigido no mesmo commit.
- **Entregue**: `ProjectFormFields` ganha `key` (input livre, placeholder "Auto"), `lead_id` (`Select` + `useWorkspaceMembers`, sentinel `__none__`, mesmo padrão de `assignee_id` em `IssueFormFields`) e `target_date` (`Input type="date"`); `CreateProjectDialog`/`EditProjectDialog` com schema Zod estendido (`key` valida `^[A-Za-z]{2,4}$`, espelhando `validate_project_key_format`); `ProjectCard.tsx` novo (badge de key, nome, badge "Arquivado" condicional, descrição truncada a 2 linhas, barra de progresso `done_issue_count/issue_count`, até 4 avatares de `member_ids` resolvidos via `useWorkspaceMembers`+`getInitials` — mesmo padrão de `ActiveProjectsWidget`, Sprint 18.5 — data de entrega, `ProjectRowActions` reaproveitado sem mudança); `ProjectsGrid.tsx` novo substitui `ProjectsTable` (deletada — único consumidor era `ProjectsListPage`) em `ProjectsListPage`; loading state trocado de `ListSkeleton` (linhas) para `CardSkeleton` (Sprint 8.5, sem consumidor até aqui) repetido no mesmo grid.
- **Não implementado (decisão explícita)**: gestão manual de membros do projeto via UI (endpoints `POST/DELETE .../projects/{id}/members` já existem desde a 19.1) — o handoff só renderiza avatares dos membros já existentes (auto-adicionados via lead, commit `f1cb90a`), nunca oferece adicionar/remover; toggle de tabs "Ativos/Arquivados" do handoff — o `Select` ALL/ACTIVE/ARCHIVED de `ProjectsToolbar` já cobre a mesma função (mesmo critério de não duplicar controle equivalente já usado nas Sprints 18.2/18.4); redesign de `ProjectDetailView` (tela "Detalhe do projeto" do handoff) — já candidato registrado em Sprint 15+.
- **Dependências**: Sprint 19.1.
- **DoD**: DoD-base — `tsc -b --noEmit`, `eslint`, `vitest`, `npm run build` verdes.

### Sprint 19.3 — M7: tela de Labels — contagem de uso (concluída)

- **Objetivo**: expor `LabelResponse.issue_count` (Sprint 19.1) na `LabelsListPage`, fechando o gap "labels mostram nº de issues" do handoff.
- **Entregue**: nova coluna "Uso" em `LabelsTable` (`{issue_count} issue(s)`, à direita, antes de Ações).
- **Não implementado (decisão explícita)**: reescrever para o padrão de edição inline por linha do handoff (swatches + inputs direto na lista) — o produto já tem um padrão de modal consolidado (`CreateLabelDialog`/`EditLabelDialog`) reaproveitado por toda a base; trocar só para esta tela quebraria a consistência sem ganho funcional (mesmo critério de não reconstruir o que já funciona usado no toggle Lista/Board, ADR-045, e nos filtros multi-seleção, Sprint 18.4).
- **Dependências**: Sprint 19.1 (independente de 19.2).
- **DoD**: DoD-base — `tsc -b --noEmit`, `eslint`, `vitest`, `npm run build` verdes.

`Projetos e Labels.dc.html` está com as três telas endereçadas (grid, líder/entrega/key no formulário, uso de label). Próxima tela do handoff: `Administracao.dc.html`.

## M7 fase 3 — gap-analysis e quebra em sub-sprints (Administração) (2026-07-21, ADR-051)

Fechada a fase 2 (Projetos e Labels), a próxima tela do handoff é `Administracao.dc.html` (Configurações/Membros/Convites do workspace). Ao contrário das fases 1 e 2, aqui a auditoria encontrou a funcionalidade **quase inteiramente pronta** desde as Sprints 4/5/10/12.3/17.1 (formulário geral, lista de membros paginada com filtro por papel e dropdown de papel, transferência de propriedade, convite por link com lista de pendentes) — a tela nunca tinha sido tocada por nenhuma sub-sprint de M7, então ainda usava a linguagem visual genérica pré-redesign. Dois gaps reais de UX/funcionalidade, além do visual:

- **Excluir workspace era confirmação só de clique** — o handoff usa "digite o slug para confirmar" para a ação mais destrutiva do produto (apaga todos os projetos/issues/membros); nenhum componente "type to confirm" existia no código.
- **Reenviar convite não existia** — o produto nunca enviou e-mail de convite automaticamente (ADR-009: convite é um link copiável), então "reenviar" não podia significar "reenviar e-mail". Nenhum endpoint de backend reemitia um convite existente.

A fase foi quebrada em duas sub-sprints: 20.1 (fecha os dois gaps reais) e 20.2 (redesign visual das 3 abas, destravado por nada em especial — só nunca feito). Achados completos em ADR-051.

### Sprint 20.1 — M7: reenvio de convite + confirmação por digitação para excluir workspace (concluída)

- **Objetivo**: fechar os dois gaps reais de `Administracao.dc.html` antes do redesign visual.
- **Entregue**: `InvitationService.resend` (backend) — reemite token+expiração de um convite pendente ou expirado (`InvitationRepository.reissue`, mutação in-place do registro já buscado), rejeita convite já aceito (`InvitationAlreadyAcceptedError`, 409 `invitation_already_accepted` — deliberadamente **não** reaproveita `InvitationExpiredError`, cuja semântica de aceite colapsa expirado+aceito no mesmo código; aqui expirado é justamente o caso que o reenvio resolve). Endpoint `POST .../invitations/{invitation_id}/resend`, mesma permissão de criar (`workspace.invite`). `TypeToConfirmDialog` novo (`shared/components/overlay/`, mesma forma de `ConfirmActionDialog` + um campo que precisa bater com um texto exato antes de habilitar o botão) usado em `WorkspaceGeneralSettings` com o slug do workspace como texto de confirmação, substituindo o `ConfirmActionDialog` genérico só ali (as demais ações destrutivas do app — remover membro, cancelar convite, sair — continuam com confirmação de clique, adequada à reversibilidade/escopo menor de cada uma). Frontend: `InviteLinkStep` extraído de `InviteMemberDialog` para um arquivo próprio (reaproveitado pelo fluxo de reenvio, que precisa mostrar o mesmo link-para-copiar); botão "Reenviar" em `WorkspaceInvitationsSettings` visível para convites `PENDING`/`EXPIRED`.
- **Dependências**: nenhuma — dois gaps independentes, ambos pré-requisito de nada em especial (não bloqueiam o visual da 20.2).
- **DoD**: DoD-base — `ruff`/`mypy` limpos; `pytest` (unit + contract) verde contra Postgres/Redis reais, incluindo os cenários novos (reenvio invalida o token antigo, rejeita convite aceito, 404 desconhecido); `tsc -b --noEmit`/`eslint`/`vitest`/`npm run build` do frontend verdes.

### Sprint 20.2 — M7: redesign visual das 3 abas de Administração (concluída)

- **Objetivo**: aplicar a linguagem visual do handoff (tokens `--panel`/`--border`/`font-heading`, mesmo padrão de `DashboardWidgetCard`/`ProjectCard` desde a Sprint 18.1) às abas Geral/Membros/Convites — sem mudar nenhum comportamento.
- **Entregue**: título da página e das duas abas em `font-heading`; formulário geral e zona de perigo envoltos em cartões `--panel`/`border-border` (zona de perigo com borda `--destructive`, mesmo tom já usado no resto do app — `--red` é a mesma variável, ADR-019); listas de membros/convites com o mesmo cartão `--panel` já usado pelas listagens de Issues/Projetos/Labels desde M7 fase 1/2.
- **Não implementado (decisão explícita)**: mover a navegação Config/Membros/Convites de `Tabs` (estrutura atual) para sub-nav na Sidebar como o handoff sugere — as três telas do handoff são arquivos/rotas separados ali, mas nesta base elas já funcionam bem como abas de uma página só; trocar a estrutura de navegação sem ganho funcional real repetiria a mesma decisão já tomada nas Sprints 18.2/18.4/19.2/19.3 de não reconstruir o que já funciona. Hint descritivo por papel no dropdown de alterar papel do handoff (ex.: "Admin: gerencia membros, convites e configurações") — cosmético, mesmo critério de corte de detalhe decorativo já aplicado em outras sub-sprints visuais.
- **Dependências**: nenhuma estrutural — só reskin. Feito depois da 20.1 por conveniência (mesmos arquivos), não por bloqueio real.
- **DoD**: DoD-base — `tsc -b --noEmit`, `eslint`, `vitest`, `npm run build` verdes.
